...
from core.scanner import scan_network, discover_range, get_local_subnet, check_root, get_network_interfaces, capture_traffic
from __main__ import __version__


def _blank_router_node(ip: str) -> dict:
    """Synthetic device node for a traceroute hop that wasn't itself scanned."""
    return {
        "ip": ip,
        "mac": "",
        "hostname": ip,
...
class ScanWorker(QThread):
    ...
    def __init__(self, subnet, service_scan=False, service_auth=False):
        super().__init__()
        self.subnet = subnet
    ...
    def run(self):
        hosts = scan_network(
            self.subnet,
            do_service_scan=self.service_scan,
...
class RangeScanWorker(QThread):
    """Arbitrary target/range discovery (local ARP + routed traceroute)."""
    finished = pyqtSignal(list)

    def __init__(self, target, service_scan=False, service_auth=False,
                 max_routed_hosts=512, probe_timeout=1.0, dedup_per_subnet=False):
        super().__init__()
        self.target = target
        self.service_scan = service_scan
        self.service_auth = service_auth
        self.max_routed_hosts = max_routed_hosts
        self.probe_timeout = probe_timeout
        self.dedup_per_subnet = dedup_per_subnet

    def run(self):
        try:
            hosts = discover_range(
                self.target,
                do_service_scan=self.service_scan,
                service_auth=self.service_auth,
                max_routed_hosts=self.max_routed_hosts,
                probe_timeout=self.probe_timeout,
                dedup_per_subnet=self.dedup_per_subnet,
            )
        except Exception:
            hosts = []
        self.finished.emit(hosts)
...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self):
        ...
        # Optional target/range — empty = local ARP scan of the selected iface.
        self.range_input = QLineEdit()
        self.range_input.setPlaceholderText("range (e.g. 10.0.0.0/24, 10.0.1.1-50)  — empty = local")
        self.range_input.setFixedWidth(330)
        ...
        """)
        self.range_input.returnPressed.connect(self._start_scan)

        self.svc_check = QCheckBox("svc")
        self.svc_check.setToolTip("Fingerprint embedded web services (iLO/InfoPrint/XPort/SATO/Zebra) and test default creds")
        self.svc_check.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self.svc_check.setStyleSheet("QCheckBox { color: #aaaaaa; font-size: 11px; } QCheckBox:hover { color: #00ff99; }")

        # --- range-scan tuning (only relevant when a routed range is given) ---
        _small_edit_css = """
            QLineEdit {
                background-color: #111111; color: #00ff99;
                border: 1px solid #222222; padding: 4px 6px; font-size: 11px;
            }
            QLineEdit:focus { border-color: #00ff99; }
        """
        self.max_hosts_input = QLineEdit("512")
        self.max_hosts_input.setFixedWidth(56)
        self.max_hosts_input.setToolTip("Max routed hosts to probe (protects huge ranges)")
        self.max_hosts_input.setStyleSheet(_small_edit_css)

        self.timeout_input = QLineEdit("1.0")
        self.timeout_input.setFixedWidth(46)
        self.timeout_input.setToolTip("Per-packet timeout in seconds (ping-sweep / traceroute)")
        self.timeout_input.setStyleSheet(_small_edit_css)

        self.dedup_check = QCheckBox("dedup")
        self.dedup_check.setToolTip("Traceroute one host per /24 and reuse the path for its neighbours (faster on big ranges)")
        self.dedup_check.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self.dedup_check.setStyleSheet("QCheckBox { color: #aaaaaa; font-size: 11px; } QCheckBox:hover { color: #00ff99; }")
        ...
        layout.addSpacing(8)
        layout.addWidget(self.svc_check)
        layout.addSpacing(8)
        layout.addWidget(self.max_hosts_input)
    ...
    def _start_scan(self):
        ...
        service_scan = self.svc_check.isChecked()
        target = self.range_input.text().strip()

        if target:
            # Arbitrary range — auto-detects local (ARP) vs routed (traceroute).
            try:
                max_hosts = max(1, int(self.max_hosts_input.text().strip() or "512"))
            except ValueError:
                max_hosts = 512
            try:
                timeout = max(0.1, float(self.timeout_input.text().strip() or "1.0"))
            except ValueError:
                timeout = 1.0
            self.subnet_label.setText(f"target: {target}")
            self.statusBar().showMessage(f"Scanning {target} ...")
            self.worker = RangeScanWorker(
                target,
                service_scan=service_scan,
                service_auth=service_scan,
                max_routed_hosts=max_hosts,
                probe_timeout=timeout,
                dedup_per_subnet=self.dedup_check.isChecked(),
            )
        else:
            iface = self.iface_selector.currentData()
            subnet = get_local_subnet(iface["name"])
            ...
            self.worker = ScanWorker(
                subnet, service_scan=service_scan, service_auth=service_scan
            )
    ...
    def _build_topology(self, hosts: list) -> dict:
        # Routed scans carry an L3 path (hops); build hierarchy from traceroute.
        if any(h.get("hops") for h in hosts):
            return self._build_topology_routed(hosts)
        try:
            iface = self.iface_selector.currentData()
    ...
    def _build_topology_routed(self, hosts: list) -> dict:
        """Topology for routed/range scans: hierarchy comes from traceroute hops.

        Each distinct hop IP becomes a synthetic 'router' node; hosts hang off
        their last-hop router; hosts are boxed by /24 subnet. Local (ARP) hosts
        in the same scan attach to the default gateway.
        """
        host_ips = {h.get("ip") for h in hosts}
        router_ips: dict[str, None] = {}   # ordered set of hop routers
        edges = []
        seen_edges = set()
        ...
        for host in hosts:
            ip = host.get("ip", "")
            path = [hp for hp in (host.get("hops") or []) if hp and hp != ip]

            if not path:
                # Local/ARP host or untraceable: hang directly off the gateway.
                continue

            # Chain the routers: gateway -> internet, then router -> prev router.
            prev = "internet"
            for hop in path:
                router_ips.setdefault(hop, None)
                add_edge(hop, prev, "uplink" if prev == "internet" else "backbone")
                prev = hop
            if gateway_ip is None:
                gateway_ip = path[0]
            add_edge(ip, prev, "client")

        # Local hosts (no usable path) attach to the gateway.
        if gateway_ip:
            for host in hosts:
                ip = host.get("ip", "")
                if ip in router_ips:
                ...
                path = [hp for hp in (host.get("hops") or []) if hp and hp != ip]
                if not path:
                    add_edge(ip, gateway_ip, "client")

        # Synthesize router nodes for hops that weren't themselves scanned hosts.
        devices = list(hosts)
        for rip in router_ips:
            if rip not in host_ips:
                router = _blank_router_node(rip)
                if rip == gateway_ip:
                    router["role"] = "gateway"
                devices.append(router)

        # Subnet bounding boxes: group real hosts by /24.
        subnets_map = {}
        for host in hosts:
            ip = host.get("ip", "")
            try:
        ...
        return {
            ...
            "subnets": list(subnets_map.values()),
            "edges": edges,
            "intermediates": list(router_ips.keys()),
        }
